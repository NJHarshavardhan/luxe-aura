...
export const Card = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { customClass?: string }>(
  ({ customClass, ...rest }, ref) => (
    <div
      ...
      {...rest}
      className={[
        "absolute top-1/2 left-1/2 rounded-2xl border border-border/60",
        "bg-gradient-to-br from-background/95 via-card/95 to-background/90",
        "shadow-xl shadow-primary/10 transform-gpu backface-hidden",
        "backdrop-blur-xl",
        customClass ?? "",
        rest.className ?? ""
      ]
        .join(" ")
        .trim()}
      style={{ ...rest.style, transformStyle: "preserve-3d", willChange: "transform" }}
    />
  )
);
...
const CardSwap = ({
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      tl.call(() => {
        ...
        if (onActiveChange && order.current.length > 0) {
  ...
  return (
    <div
      ...
      style={{ width, height, perspective: 900, transformOrigin: 'bottom right' }}
      onTouchStart={(e) => {

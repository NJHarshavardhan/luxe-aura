...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      camera: THREE.PerspectiveCamera; scene: THREE.Scene; fogUniforms: any; clock: any;
      ...
      constructor(cont: HTMLElement, options: any) {
        ...
        this.renderer.setSize(Math.max(1, cont.offsetWidth), Math.max(1, cont.offsetHeight), false);
        this.renderer.setPixelRatio(window.devicePixelRatio);
        this.composer = new EffectComposer(this.renderer);
        cont.append(this.renderer.domElement);
        this.camera = new THREE.PerspectiveCamera(options.fov, Math.max(1, cont.offsetWidth) / Math.max(1, cont.offsetHeight), 0.1, 10000);
      ...
      onWindowResize() {
        const w = Math.max(1, this.container.offsetWidth);
        const h = Math.max(1, this.container.offsetHeight);
      ...
      init() {
        this.initPasses(); this.road.init(); this.leftCarLights.init();
    ...
    options.distortion = distortions[options.distortion as unknown as string] || distortions['turbulentDistortion'];
  ...
  return <div ref={containerRef} style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'absolute' }} />;
};
